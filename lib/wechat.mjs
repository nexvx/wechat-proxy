...
const CACHE_DIR = path.join(os.homedir(), '.config', 'wechat-proxy')
 ...
 */
async function remoteGetAccessToken(proxy, appName = 'default') {
 ...
 * Server Mode: 上传图片素材
 */
async function remoteUploadMaterial(proxy, filePath, timeout, appName = 'default') {
  const boundary = crypto.randomBytes(16).toString('hex')
  const body = buildMultipartBody(filePath, boundary, appName)
 ...
 * Server Mode: 创建草稿
 */
async function remoteCreateDraft(proxy, articles, label, timeout, appName = 'default') {
  const body = JSON.stringify({ articles, label, appid: appName })
 ...
 * 获取 Access Token（带缓存）
 */
export async function getAccessToken(cfg, appName = 'default') {
  if (cfg.proxy) return remoteGetAccessToken(cfg.proxy, appName)
  const appConfig = cfg.wechatApps?.[appName] || { appid: cfg.wechatAppID, secret: cfg.wechatSecret }
  if (!appConfig.appid || !appConfig.secret) {
    throw new Error(`未找到 app "${appName}" 的配置`)
  }

  const cacheKey = `${appConfig.appid}:${appConfig.secret}`
  const now = Math.floor(Date.now() / 1000)
  if (tokenCache[cacheKey] && now < tokenCache[cacheKey].expiresAt - 60) {
    return tokenCache[cacheKey].token
  }

  const url = `https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=${appConfig.appid}&secret=${appConfig.secret}`
  const raw = await makeRequest(url)
  const data = JSON.parse(raw)
...
/**
 * 构建 multipart/form-data body（包含 appName）
 */
function buildMultipartBody(filePath, boundary, appName = 'default') {
  const filename = path.basename(filePath)
  const ext = path.extname(filename).toLowerCase().slice(1)
  const mimeMap = { jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png', gif: 'image/gif', webp: 'image/webp' }
  const mime = mimeMap[ext] || 'application/octet-stream'

  const appNamePart = Buffer.from(
  ...
  return Buffer.concat([appNamePart, fileHeader, fileData, footer])
}

/**
 * 上传图片素材到微信
 */
export async function uploadMaterial(cfg, filePath, appName = 'default') {
  if (cfg.proxy) return remoteUploadMaterial(cfg.proxy, filePath, cfg.httpTimeout, appName)
 ...
 * 带重试的上传
 */
export async function uploadMaterialWithRetry(cfg, filePath, appName = 'default', maxRetries = 3) {
  let lastErr
  for (let i = 0; i < maxRetries; i++) {
    try {
      return await uploadMaterial(cfg, filePath, appName)
    } catch (err) {
      lastErr = err
 ...
 */
export async function uploadMaterialCached(cfg, filePath, { useCache = true } = {}, appName = 'default') {
  if (useCache) {
    ...
    const result = await uploadMaterialWithRetry(cfg, filePath, appName)
    cache[md5] = result
    saveCache(cache)
    return result
  }
  return uploadMaterialWithRetry(cfg, filePath, appName)
}
 ...
 */
export async function uploadMaterialBatch(cfg, filePaths, { concurrency = 3, useCache = true } = {}, appName = 'default') {
  const results = []
  // 分批并发，避免触发微信频率限制
  for (let i = 0; i < filePaths.length; i += concurrency) {
    ...
    const batchResults = await Promise.all(
      batch.map(async (fp) => {
        try {
          const r = await uploadMaterialCached(cfg, fp, { useCache }, appName)
 ...
 */
export async function createDraft(cfg, articles, label = '图文草稿', appName = 'default') {
  if (cfg.proxy) return remoteCreateDraft(cfg.proxy, articles, label, cfg.httpTimeout, appName)
